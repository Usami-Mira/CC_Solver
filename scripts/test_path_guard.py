...
with tempfile.TemporaryDirectory() as td:
    ...
    rc, err = call("Bash", {"command": "python3 -c \"x = 1 / 2; print(x)\""}, ws)
    check("Bash inline python division allowed (bare / = operator)", rc == 0, f"rc={rc} {err}")
    rc, err = call("Bash", {"command": f"ls {ws} && python3 -c \"a = 3 / 4\""}, ws)
    check("Bash division in chained command allowed (segment-aware)", rc == 0, f"rc={rc} {err}")
    rc, err = call("Bash", {"command": "ls /"}, ws)
    check("Bash ls / denied (root scan)", rc == 2, f"rc={rc} {err}")
    rc, err = call("Bash", {"command": "find / -name '*.md'"}, ws)
    check("Bash find / denied (root scan)", rc == 2, f"rc={rc} {err}")
    rc, err = call("Bash", {"command": "grep -r answer /"}, ws)
    check("Bash grep ... / denied (root scan)", rc == 2, f"rc={rc} {err}")
